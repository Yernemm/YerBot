                ...
                case "link":
                    ...
                    if (argsArr[1]) {
                        if (argsArr[1].length >= 2 && argsArr[1].length <= 15) {
                                    ...
                                    sql.run(`CREATE TABLE IF NOT EXISTS lfAcc (userId TEXT, lfUser TEXT)`)
                                        .then(() => {
                                            sql.run(`INSERT INTO lfAcc (userId, lfUser) VALUES (?, ?)`, [message.author.id, argsArr[1]])
                                                .then(() => {
                                                    m.logSend(config, client, message, `Discord account linked to "${argsArr[1]}"`);
                                                })
                                                .catch(err => {
                            ...
                            m.logSend(config, client, message, `Specify a valid Last.FM account name.`);
                        }
                    } else {
                        m.logSend(config, client, message, `Specify a Last.FM account name.`);
                    }
                    ...
                    sql.get(`SELECT * FROM lfAcc WHERE userId ="${message.author.id}"`)
                        .then(row => {
                            ...
                            sql.run(`DELETE FROM lfAcc WHERE userId ="${message.author.id}"`)
                                .then(() => {
                                    m.logSend(config, client, message, `LF user "${username}" has been unlinked.`);
                                })
                                .catch(err => {
